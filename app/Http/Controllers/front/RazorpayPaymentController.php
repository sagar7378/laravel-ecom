            ...
            if ($order_id) {
                ...
                foreach ($cart_data as $item) {
                    $order_item = [
                        'order_id' => $order_id,
                        'customer_id' => $customer_id,
                        'food_id' => $item['food_id'], // Assuming 'food_id' is a foreign key and can be null
                        'price' => $item['food_price'], // Assuming 'food_id' is a foreign key and can be null
                        'quantity' => $item['food_qty'],
                        'subtotal' => $item['food_price'] * $item['food_qty'],
                    ];

                    // Insert each order item into the database
                  $orders_items =   DB::table('order_items')->insert($order_item);
                }
